...
def propagation_and_random_search(source_patches, target_patches,
    ...
    if best_D is None:
        best_D = np.empty((row, col)) * np.nan

    r_list = []
    power = 0
    temp = w * alpha ** power
    while temp >= 1:
        r_list.append(temp * np.random.randint(low=-1, high=2, size=new_f.shape))
        power += 1
        temp = w * alpha ** power
    r_np = np.array(r_list)


    if odd_iteration: # start with the top left patch
        start_x = 0
        start_y = 0
        end_x = row
        end_y = col
        n = 1
    else: # start with the bottom right patch
        start_x = row - 1
        start_y = col -1
        end_x = -1
        end_y = -1
    ...
    for i in range(start_x, end_x, n):
        for j in range(start_y, end_y, n):
            offset = [new_f[i, j]]
            if propagation_enabled:
                if 0 <= i - n <= source_patches.shape[0] - 1:
                    offset.append(new_f[i - n, j])
                if 0 <= j - n <= source_patches.shape[1] - 1:
                    offset.append(new_f[i, j - n])
            offset = np.array(offset)
            if random_enabled:
                random_offsets = new_f[i, j] + r_np[:, i, j, :]
                offset = np.concatenate((offset, random_offsets))

            target_set = offset + [i, j]
            target_x, target_y = np.clip(target_set[:, 0], 0, source_patches.shape[0] - 1).astype(np.uint),\
                                 np.clip(target_set[:, 1], 0, source_patches.shape[1] - 1).astype(np.uint)

            Dlist = np.sum(np.nan_to_num((target_patches[target_x, target_y] - source_patches[i, j]) ** 2), axis=(1,2))
            best_D[i, j] = Dlist[np.argmin(Dlist)]

            new_f[i, j] = offset[np.argmin(Dlist)]
...
def reconstruct_source_from_target(target, f):
    ...
    ###  PLACE YOUR CODE BETWEEN THESE LINES  ###
    #############################################
    temp = make_coordinates_matrix(target.shape) + f

    rec_source = target[np.clip(temp[:, :, 0], 0, target.shape[0] - 1),
                  np.clip(temp[:, :, 1], 0, target.shape[1] - 1)]
